    ...
    @synchronized(self)     {
        if(!view_font)
        {
            if(_screenType==UIInterfaceOrientationLandscapeLeft||_screenType==UIInterfaceOrientationLandscapeRight)
                view_font=[[FGFont alloc]initWithFrame:CGRectMake(0, 0, H, W)];
            if(_screenType==UIInterfaceOrientationPortrait||_screenType==UIInterfaceOrientationPortraitUpsideDown)
                view_font=[[FGFont alloc]initWithFrame:CGRectMake(0, 0, W, H)];
            ...
            for(NSString *str_fontname in arr_fontname)
            {
                ...
                UILabel *lb=[[UILabel alloc]initWithFrame:CGRectMake(0, nextHeight, W, _size.height)];
                lb.backgroundColor=[UIColor clearColor];
                lb.textColor=[UIColor whiteColor];
                lb.font=font;
                nextHeight+=lb.frame.size.height+5;
                lb.text=[NSString stringWithFormat:@"测试中文 哈哈哈%@",str_fontname];
                [sv_font addSubview:lb];
                [lb release];
            }
            sv_font.contentSize=CGSizeMake(W, nextHeight);
