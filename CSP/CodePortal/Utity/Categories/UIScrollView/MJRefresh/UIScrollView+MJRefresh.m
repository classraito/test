...
#pragma mark - header
static const char MJRefreshLeftKey = '\0';
- (void)setMj_left:(MJRefreshFooter *)mj_left
{
  if (mj_left != self.mj_left) {
    // 删除旧的，添加新的
    [self.mj_left removeFromSuperview];
    [self insertSubview:mj_left atIndex:0];
    
    // 存储新的
    [self willChangeValueForKey:@"mj_left"]; // KVO
    objc_setAssociatedObject(self, &MJRefreshLeftKey,
                             mj_left, OBJC_ASSOCIATION_ASSIGN);
    [self didChangeValueForKey:@"mj_left"]; // KVO
  }
}

- (MJRefreshFooter *)mj_left
{
  return objc_getAssociatedObject(self, &MJRefreshLeftKey);
}
