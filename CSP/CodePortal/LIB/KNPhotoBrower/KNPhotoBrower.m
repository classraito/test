...
//  如果传入的 ActionSheetArr 有下载图片这一选项. 则在这里调用和下面一样的方法 switch.....,如果没有下载图片,则通过代理方法去实现...
        ...
        //TODO: rui.gong fixed below code
        KNActionSheet *actionSheet = [[KNActionSheet alloc] initWithCancelTitle:nil otherTitleArr:@[multiLanguage(@"Save To Photo Album")]  actionBlock:^(NSInteger buttonIndex) {
            // 让代理知道 是哪个按钮被点击了
            if([weakSelf.delegate respondsToSelector:@selector(photoBrowerRightOperationActionWithIndex:)]){
                [weakSelf.delegate photoBrowerRightOperationActionWithIndex:buttonIndex];
            }
...
- (void)dismiss{
    ...
    tempView.contentMode = UIViewContentModeScaleAspectFill;//TODO:  rui.gong fix it
    tempView.clipsToBounds = YES;//TODO:  rui.gong fix it
    SDWebImageManager *mgr = [SDWebImageManager sharedManager];
    ...
    if(rect.origin.y > ScreenHeight ||
       ...
       rect.origin.x <= -rect.size.width
        ...
        [UIView animateWithDuration:PhotoBrowerBrowerTime delay:0 options:UIViewAnimationOptionCurveEaseOut animations:^{
            ...
            [self removeFromSuperview];
            // 让 代理 知道 PhotoBrower 即将 消失
            if([self.delegate respondsToSelector:@selector(photoBrowerDidDismiss)]){
                [self.delegate photoBrowerDidDismiss];
            }
        }];
