...
- (void)setIsAheadInitImages:(BOOL)isAheadInitImages {
  self.localizationImagesGroup = nil;
  _isAheadInitImages           = isAheadInitImages;
  if (isAheadInitImages) {
    self.localizationImagesGroup = [[NSMutableArray alloc] init];
    for (int i = 0; i < self.localizationImageNamesGroup.count; i++) {
      UIImage *img = IMGWITHNAME(self.localizationImageNamesGroup[i]);
      [self.localizationImagesGroup addObject:img];
    }
  }
}
...
- (void)setupPageControl {
  ...
  int indexOnPageControl =
      [self pageControlIndexWithCurrentCellIndex:[self currentIndex]];
  ...
  switch (self.pageControlStyle) {
    case SDCycleScrollViewPageContolStyleAnimated: {
      ...
      pageControl.bottomHeight           = self.bottomHeight;
...
- (void)layoutSubviews {
  ...
  if ([self.pageControl isKindOfClass:[TAPageControl class]]) {
    ...
    size = CGSizeMake(self.imagePathsGroup.count *
                          self.pageControlDotSize.width * 1.5,
...
//        UIImage *_img_sub = [image subImageWithSquareWithInSize:CGSizeMake(320 * ratioW, 160 * ratioH)];
        cell.imageView.image = image;
      }];
    } else {
      UIImage *image = nil;
      if (self.isAheadInitImages) {
        image = self.localizationImagesGroup[itemIndex];
      } else {
  ...
  } else if (!self.onlyDisplayText &&
             [imagePath isKindOfClass:[UIImage class]]) {
...
- (void)collectionView:(UICollectionView *)collectionView
  ...
  if ([self.delegate respondsToSelector:@selector(cycleScrollView:
    ...
    [self.delegate cycleScrollView:self
              didSelectItemAtIndex:
                  [self pageControlIndexWithCurrentCellIndex:indexPath.item]];
  }
  if (self.clickItemOperationBlock) {
    self.clickItemOperationBlock(
        [self pageControlIndexWithCurrentCellIndex:indexPath.item]);
  }
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
  ...
  int indexOnPageControl =
      [self pageControlIndexWithCurrentCellIndex:itemIndex];
