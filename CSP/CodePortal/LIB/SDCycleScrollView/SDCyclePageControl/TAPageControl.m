...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  if (touch.view != self) {
    ...
    if ([self.delegate respondsToSelector:@selector(TAPageControl:
                                              didSelectPageAtIndex:)]) {
...
/**
 *  Will update dots display and frame. Reuse existing views or instantiate one
 * if required. Update their position in case frame changed.
 */
- (void)updateDots {
...
/**
 *  Update frame control to fit current number of pages. It will apply required
 * size if authorize and required.
 *
 *  @param overrideExistingFrame BOOL to allow frame to be overriden. Meaning
 * the required size will be apply no mattter what.
 */
...
- (void)updateDotFrame:(UIView *)dot atIndex:(NSInteger)index {
  ...
  CGFloat x = (self.dotSize.width + self.spacingBetweenDots) * index +
  ...
  dot.frame =
      CGRectMake(x, y - _bottomHeight, self.dotSize.width, self.dotSize.height);
}
...
- (UIView *)generateDotView {
  ...
  if (self.dotViewClass) {
    dotView = [[self.dotViewClass alloc]
        initWithFrame:CGRectMake(0, 0, self.dotSize.width,
                                 self.dotSize.height)];
...
- (void)changeActivity:(BOOL)active atIndex:(NSInteger)index {
  if (self.dotViewClass) {
    TAAbstractDotView *abstractDotView =
        (TAAbstractDotView *)[self.dots objectAtIndex:index];
    if ([abstractDotView respondsToSelector:@selector(changeActivityState:)]) {
      [abstractDotView changeActivityState:active];
    } else {
      NSLog(@"Custom view : %@ must implement an 'changeActivityState' method "
            @"or you can subclass %@ to help you.",
