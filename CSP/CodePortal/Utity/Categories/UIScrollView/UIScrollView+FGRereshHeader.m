...
#define SPINNERWIDTH 40
#define SPINNERHEIGHT 40
#define TAG_VIEWWAITING 1000
...
#pragma mark - UIScrollView (FGRereshHeader)
...
//    UIView *view_bg = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
//    view_bg.backgroundColor = [UIColor clearColor];
//    view_bg.tag = TAG_VIEWWAITING;
//    [self addSubview:view_bg];
//    
//    UIView *view_blackbg = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
//    view_blackbg.backgroundColor = [UIColor colorWithRed:237/255.0 green:237/255.0 blue:237/255.0 alpha:1.0];
//    view_blackbg.alpha = 1.0f;
//    [view_bg addSubview:view_blackbg];
//    
//    CGRect rect_spinnerView = CGRectMake(self.bounds.size.width/2-SPINNERWIDTH/2, self.bounds.size.height/2-SPINNERHEIGHT/2, SPINNERWIDTH, SPINNERHEIGHT);
//    view.spinnerView.frame = rect_spinnerView;
//    [view_bg addSubview:view.spinnerView];
//    [view.spinnerView stopAnimating];
//    [view_bg setHidden:YES];
  }
}
...
- (void)setupPullToRefreshViewWithStopTitle:(NSString *)stopTitle
  ...
  self.pullToRefreshView.titles = [NSMutableArray
      arrayWithObjects:stopTitle, triggeredTitle, loadingTitle, nil];
}
...
- (void)setupSpinnerStyle:(UIColor *)color lineWidth:(CGFloat)width {
  if (self.pullToRefreshView.spinnerView == nil||color == nil || width < 0)
    return;
  
  self.pullToRefreshView.spinnerView.lineWidth = width;
  self.pullToRefreshView.spinnerView.tintColor = color;
}

- (void)setupSpinnerStyleWithColor:(UIColor *)color {
  CGFloat width = self.pullToRefreshView.spinnerView.lineWidth;
  [self setupSpinnerStyle:color lineWidth:width];
}

- (void)setupSpinnerStyleWithLineWidth:(CGFloat)width {
  UIColor * color = self.pullToRefreshView.spinnerView.tintColor;
  [self setupSpinnerStyle:color lineWidth:width];
}

- (void)setupLoadingMaskLayerHidden:(BOOL)isHidden withAlpha:(CGFloat)alpha withSpinnerHidden:(BOOL)spinnerHidden {
  
  [self setupLoadingMaskLayerHidden:isHidden withAlpha:alpha];
  self.pullToRefreshView.spinnerView.hidden = spinnerHidden;
}

- (void)setupLoadingMaskLayerHidden:(BOOL)isHidden withAlpha:(CGFloat)alpha {
  UIView *view_bg = [self viewWithTag:TAG_VIEWWAITING];
  if (view_bg == nil && isHidden == NO) {
    view_bg = [self initLoadingMask];
  }
  view_bg.hidden = isHidden;
  ...
  CGPoint point_offset = [self contentOffset];
  view_bg.frame = CGRectMake(0, point_offset.y, view_bg.frame.size.width, view_bg.frame.size.height);
  
  NSLog(@"self.pullToRefreshView.==%@",self.pullToRefreshView);
  if (isHidden) {
    self.scrollEnabled = YES;
    [self.pullToRefreshView.spinnerView stopAnimating];
  } else {
    self.scrollEnabled = NO;
    [self.pullToRefreshView.spinnerView startAnimating];
  }
  self.pullToRefreshView.spinnerView.hidden = NO;
}

- (void)setupLoadingMaskLayerHidden:(BOOL)isHidden{
  [self setupLoadingMaskLayerHidden:isHidden withAlpha:1.0];
}

- (UIView *)initLoadingMask {
  UIView *view_bg = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height)];
  view_bg.backgroundColor = [UIColor clearColor];
  view_bg.tag = TAG_VIEWWAITING;
  [self addSubview:view_bg];
  ...
  CGRect rect_spinnerView = CGRectMake(self.bounds.size.width/2-SPINNERWIDTH/2, self.bounds.size.height/2-SPINNERHEIGHT/2, SPINNERWIDTH, SPINNERHEIGHT);
  self.pullToRefreshView.spinnerView.frame = rect_spinnerView;
  [view_bg addSubview:self.pullToRefreshView.spinnerView];
  [self.pullToRefreshView.spinnerView stopAnimating];
  [view_bg setHidden:YES];
...
- (void)drawRect:(CGRect)rect {
  ...
  if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 5) {
    NSArray *alphaGradientColors = [NSArray
        arrayWithObjects:(id)[self.arrowColor colorWithAlphaComponent:0]
                             .CGColor,
                         (id)[self.arrowColor colorWithAlphaComponent:1]
                             .CGColor,
                         nil];
