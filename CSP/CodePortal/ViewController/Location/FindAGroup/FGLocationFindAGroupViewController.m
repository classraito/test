//
//  FGLocationFindAGroupViewController.m
//  CSP
//
...
@interface FGLocationFindAGroupViewController ()
{
    
}
@end

@implementation FGLocationFindAGroupViewController
...
- (void)viewDidLoad {
    ...
    self.view_topPanel.str_title = multiLanguage(@"JOIN A GROUP");
...
-(void)internalInitalMapView
{
    if(view_map)
        return;
    view_map = (FGLocationJoinAGroupMapView *)[[[NSBundle mainBundle] loadNibNamed:@"FGLocationJoinAGroupMapView" owner:nil options:nil] objectAtIndex:0];
    ...
    [view_map.cb_myGroup.button addTarget:self action:@selector(buttonAction_myGroup:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)internalInitalListView
{
    if(view_list)
        return;
    view_list = (FGLocationJoinAGroupListView *)[[[NSBundle mainBundle] loadNibNamed:@"FGLocationJoinAGroupListView" owner:nil options:nil] objectAtIndex:0];
...
-(void)receivedDataFromNetwork:(NSNotification *)_notification
{
    ...
    if ([HOST(URL_LOCATION_GroupList) isEqualToString:_str_url]) {
        if([[_dic_requestInfo allKeys] containsObject:@"GetGroupList_loadMore"])
        {
            [view_list loadMoreGroupList];
        }
        else
        {
...
-(void)buttonAction_myGroup:(id)_sender
{
    ...
    FGLocationMyGroupViewController *vc_locationMyGroup = [[FGLocationMyGroupViewController alloc] initWithNibName:@"FGLocationMyGroupViewController" bundle:nil];
    [manager pushController:vc_locationMyGroup navigationController:nav_current];
}
@end
