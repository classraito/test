...
- (BOOL)preCheckWXAppInstalled {
  ...
  if (!retVal) {
    [commond alert:multiLanguage(@"ALERT") message:multiLanguage(@"WeChat is not installed!") callback:^(FGCustomAlertView *alertView, NSInteger buttonIndex) {
    }];
  } //TODO: alert here
...
- (void)loginInViewController:(UIViewController *)_controller withCompletetionHandler:(CompletetionHandler)handler {
  [self loginInViewController:_controller];
...
- (void)refreshAccessToken {
  NSString *str_refreshToken = [NSString stringWithFormat:@"%@",[commond getUserDefaults:KEYCHAIN_KEY_REFRESHTOKEN_WECHAT]];
  if (str_refreshToken) {
...
- (void)didReceiveCode:(NSMutableDictionary *)_dic_receivedData {
  if (ISNULLObj(_dic_receivedData))
    return;
...
//  [commond saveToKeyChain:KEYCHAIN_KEY_UNIONID_WECHAT passwd:str_unionid]; //将以上重要信息保存到keychain中
  ...
  [commond setUserDefaults:str_accessToken forKey:KEYCHAIN_KEY_ACCESSTOKEN_WECHAT];
  [commond setUserDefaults:str_openid forKey:KEYCHAIN_KEY_OPENID_WECHAT];
  [commond setUserDefaults:str_refreshToken forKey:KEYCHAIN_KEY_REFRESHTOKEN_WECHAT];
  [commond setUserDefaults:str_unionid forKey:KEYCHAIN_KEY_UNIONID_WECHAT];
...
- (void)didReceiveUserInfo:(NSMutableDictionary *)_dic_receivedData {
  ...
  if (!_dic_receivedData) {
    ...
    goto END;
  } else if ([_dic_receivedData count] <= 0) {
    err = [NSError errorWithDomain:@"获取用户信息失败!" code:-1 userInfo:nil];
    goto END; //安全检查 防止崩溃
  } else {
    NSString *str_openid  = [_dic_receivedData objectForKey:@"openid"];
...
//    [commond saveToKeyChain:KEYCHAIN_KEY_UNIONID_WECHAT passwd:str_unionid];
    ...
    if (jsonData) {
      ...
      NSMutableDictionary *_dic_userinfo_wechat = [str_userinfo_json mutableObjectFromJSONString];
...
//      [_dic_userinfo setObjectSafty:@"" forKey:@"Birthday"];
      ...
      NSString *_str_json = str_userinfo_json;//[_dic_userinfo JSONString];
//      [commond saveToKeyChain:KEYCHAIN_KEY_WECHAT_USERINFO passwd:_str_json];
      [commond setUserDefaults:_str_json forKey:KEYCHAIN_KEY_WECHAT_USERINFO];

      [str_userinfo_json release];
      str_userinfo_json = nil;
    }
    info = @{ @"openid" : str_openid,
              @"accesstoken" : [commond getUserDefaults:KEYCHAIN_KEY_ACCESSTOKEN_WECHAT] };
    goto END;
  }
